...
function ActiveStocksScreen({ runtime }: { runtime: ClinicRuntime }) {
  ...
  const route = (id: string) => id === 'sale' ? '/sale' : id === 'calendar' ? '/calendar' : id === 'clients' ? '/clients' : id === 'setup' ? '/setup' : '/stocks';
  ...
  return <main className={styles.root} data-locale={locale} data-testid="stocks-root" lang={locale === 'zh' ? 'zh-Hans' : locale}>
    <AppShell activeTab="stocks" brand={t('brand.name')} location={t('brand.location')} logoutLabel={t('shell.logout')} switchUserLabel={t('shell.switchUser')} onSwitchUser={() => { runtime.session.switchUser(); router.push('/login'); }} storageAttention={runtime.storageDiagnostics.state().kind === 'granted' ? undefined : t('shell.storageAttention')} onLogout={() => { void runtime.outbox.status().then((next) => { if (next.pendingCount > 0 || next.attentionCount > 0) enqueue(t('auth.logout.blocked')); else { void runtime.session.logout(); router.push('/login'); } }); }} onTabChange={(id) => router.push(id === 'today' ? '/' : route(id))} sync={{ label: t('sync.synced'), state: status.state, count: 0, onClick: () => { void runtime.refreshSync().then(refresh); } }} tabs={[{ id: 'today', label: t('shell.tab.today') }, { id: 'calendar', label: t('shell.tab.calendar') }, { id: 'clients', label: t('shell.tab.clients') }, { id: 'sale', label: t('shell.tab.sale') }, { id: 'stocks', label: t('shell.tab.stocks') }, { id: 'setup', label: t('shell.tab.setup') }]} userName={identity.name} userRole={identity.role === 'admin' ? t('auth.role.admin') : t('auth.role.staff')}>
      <div className={styles.content}><div className={styles.toolbar}><h1>{t('stocks.title')}</h1><div className={styles.actions}><Input onChange={(event) => setQuery(event.target.value)} placeholder={t('sale.search')} value={query} /><Button data-testid="add-product-open" onClick={() => setAddOpen(true)}>{t('stocks.add')}</Button></div></div><div className={styles.table}><table><thead><tr><th>{t('stocks.name')}</th><th>{t('stocks.category')}</th><th>{t('stocks.type')}</th><th>{t('stocks.cost')}</th><th>{t('stocks.price')}</th><th>{t('stocks.margin')}</th><th>{t('stocks.stock')}</th><th /></tr></thead><tbody>{visible.map((product) => { const details = stockTableDetails(product); const margin = marginPct(product.cost, product.price); return <tr data-testid={`stock-row-${product.id}`} key={product.id}><td><span className={styles.productName}><strong>{product.name}</strong>{product.soldBy === 'weight' ? <Tag tone="blue">{t('stocks.weight')}</Tag> : null}</span><br /><small>{product.barcode ?? '—'}</small></td><td>{product.category}</td><td><Tag tone={details.typeTone}>{product.stockType}</Tag></td><td>{product.cost === 0 ? '—' : `${fmtMMK(product.cost)}${product.soldBy === 'weight' && product.unitLabel !== null ? ` / ${product.unitLabel}` : ''}`}</td><td>{product.price === 0 ? '—' : `${fmtMMK(product.price)}${product.soldBy === 'weight' && product.unitLabel !== null ? ` / ${product.unitLabel}` : ''}`}</td><td className={details.margin === 'high' ? styles.marginHigh : details.margin === 'medium' ? styles.marginMedium : details.margin === 'low' ? styles.marginLow : undefined}>{margin === null ? '—' : `${margin}%`}</td><td><span className={styles.stockValue}>{product.stockQty}{product.soldBy === 'weight' && product.unitLabel !== null ? ` ${product.unitLabel}` : ''}{details.lowStock ? <Tag tone="low">{t('tag.low')}</Tag> : null}</span>{details.lotLines.map((line) => <small className={styles.lotLine} key={line}>{line}</small>)}</td><td className={styles.actions}>{pendingIds.has(product.id) ? <span data-testid={`product-waiting-${product.id}`} className={styles.notice}>{t('stocks.pending')}</span> : <Button data-testid={`product-edit-${product.id}`} onClick={() => requestEdit(product)} pill size="sm" variant="ghost">{t('stocks.edit')}</Button>}<Button data-testid={`receive-open-${product.id}`} onClick={() => { setReceiveProduct(product); setReceiveQty('1'); setReceiveCost(''); setReceiveLot(''); setReceiveExpiry(''); }} pill size="sm" variant="ghost">{t('stocks.receive')}</Button></td></tr>; })}</tbody></table></div></div>
    </AppShell>
